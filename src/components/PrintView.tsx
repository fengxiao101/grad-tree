...
import { X, Printer, LayoutGrid, LayoutList, Share2, Check, Copy } from 'lucide-react';
...
import type { MetaRequirement } from '../data/majorSchema';
import {
  computeAssignments,
  cardSatisfiesWim,
  countSectionSlots,
            ...
            <section className="mb-8">
              {(() => {
                ...
                const trackUnits = activeCotermTrack
                  ? calculateRequirementUnits(activeCotermTrack.sections, cotermAssignments, cotermFills, enrolledCards)
                  : 0;
                const sectionUnitsComplete = cotermSections.every(section =>
                  !section.minUnits || calculateRequirementUnits([section], cotermAssignments, cotermFills, enrolledCards) >= section.minUnits
                );
                  ...
                  <>
                    ...
                    <div className="space-y-3">
                      {cotermSections.map(section => {
                        ...
                        const hasRequirement = sectionHasRequirement(section);
                        const units = section.minUnits ? calculateRequirementUnits([section], cotermAssignments, cotermFills, enrolledCards) : 0;
                        const sComplete = hasRequirement
                          && (sNeeded === 0 || sFilled >= sNeeded)
...
export function PrintView({ onClose }: Props) {
  ...
  const sharePlatforms = buildShareTargets({
    message: shareMsg,
